...
export function decodePng(buf) {
  ...
  // Обрезанный ответ иначе доходил до разбора и падал с «неизвестный фильтр undefined».
  const expected = height * (stride + 1);
  if (raw.length < expected) throw new Error(`PNG оборван: ${raw.length} байт вместо ${expected}`);
