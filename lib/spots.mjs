...
export async function buildSpots(passes, log = () => {}) {
  ...
  const spots = [];
  // Леса, по которым прошла гроза — отдельным слоем на карту, без дублей.
  // Тайл режет лес по своей границе, поэтому соседние куски рисуем рядом:
  // без обводки они складываются в один сплошной массив.
  const hitForests = new Map();
  let idSeq = 0;
  ...
  groupByTime(passes).forEach((storm, si) => {
    ...
    for (const cell of storm.items) {
      ...
      if (forest) {
        inForest.push({ ...cell, forest });
        if (!hitForests.has(forest.id)) hitForests.set(forest.id, forest);
      }
    }
  ...
  return {
    spots: spots.sort((a, b) => b.acc - a.acc),
    forests: [...hitForests.values()].map((f) => ({
      id: f.id,
      ring: simplifyRing(f.ring, 0.0006).map(([lon, lat]) => [+lon.toFixed(5), +lat.toFixed(5)]),
    })),
  };
}
