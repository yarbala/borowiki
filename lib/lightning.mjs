...
import { BBOX, GRID, WMS } from './config.mjs';
...
async function writeDay(day, data) {
  ...
  await fs.writeFile(path.join(FRAMES_DIR, `${day}.json`), JSON.stringify(data));
}
    ...
    try {
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          ...
          lit.push([x, y, Math.round(colorToIntensity(data[i], data[i + 1], data[i + 2]) * 100)]);
        }
      }
      return lit;
    } catch (e) {
...
export async function loadFrames(from, to, { concurrency = 6, onProgress } = {}) {
  ...
  const missing = times.filter((t) => byDay.get(dayOf(t))[hhmmOf(t)] === undefined);
  const failed = [];
  let done = 0;
  ...
  const worker = async () => {
    while (queue.length) {
      const t = queue.shift();
      try {
        byDay.get(dayOf(t))[hhmmOf(t)] = await fetchFrame(t);
      } catch (e) {
  ...
  const frames = times
    .map((t) => ({ time: t, lit: byDay.get(dayOf(t))[hhmmOf(t)] }))
    .filter((f) => Array.isArray(f.lit) && f.lit.length > 0);

  return { frames, fetched: missing.length - failed.length, cached: times.length - missing.length, failed };
}
