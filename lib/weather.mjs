...
/**
 * Сетка осадков по региону — контроль для проверки гипотезы:
 * видно, где дождь был такой же, но молний не было.
 * @returns {Promise<{stepDeg:number, cells:Array<[number,number,number]>}>} [lat, lon, мм за окно]
 */
export async function fetchRainGrid(bbox, { stepDeg = 0.2, from, to } = {}) {
  ...
  for (let i = 0; i < points.length; i += BATCH) {
    ...
    chunk.forEach((p, j) => {
      ...
      if (!daily) return;
      const mm = daily.reduce((a, v) => a + (v ?? 0), 0);
      if (mm >= 1) cells.push([p.lat, p.lon, Math.round(mm * 10) / 10]);
    });
