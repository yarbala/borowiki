...
export async function fetchDaily(points, { today = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  for (let i = 0; i < missing.length; i += BATCH) {
    ...
    for (let attempt = 1; attempt <= 4; attempt++) {
      ...
      if (res.status === 429) {
        // Суточный лимит ждать бессмысленно — он снимется только завтра.
        const body = await res.text();
        if (/daily/i.test(body)) throw new Error('исчерпан суточный лимит Open-Meteo — повторите завтра');
        await sleep(61_000);
        continue;
      }
...
/**
 * Сетка осадков по региону — контроль для проверки гипотезы: видно, где дождь
 ...
 * @returns {Promise<{stepDeg:number, cells:Array<[number,number,number,number,number]>}>}
 *   [lat, lon, мм в сильнейший день, суток назад, мм за всё окно]
 */
export async function fetchRainGrid(bbox, { stepDeg = 0.1, from, to, keep = () => true } = {}) {
  const points = [];
  ...
  // Осадки суточные — за день не меняются, поэтому кэшируем по окну.
  const cacheFile = path.join(CACHE_DIR, `rain-${fromDate}_${toDate}-${stepDeg}.json`);
  try {
  ...
  for (let i = 0; i < points.length; i += BATCH) {
    ...
    chunk.forEach((p, j) => {
      ...
      if (maxMm < 1) return;
      const agoDays = maxIdx >= 0 && days[maxIdx]
        ? Math.max(0, Math.round((Date.parse(`${toDate}T12:00:00Z`) - Date.parse(`${days[maxIdx]}T12:00:00Z`)) / 86400_000))
        : 0;
      cells.push([p.lat, p.lon, Math.round(maxMm * 10) / 10, agoDays, Math.round(total * 10) / 10]);
    });

    // Лимит считается взвешенно: сотня точек — это ~100 вызовов из 600 в минуту.
    // Разводим пачки по времени, иначе шесть подряд упираются в потолок.
    if (i + BATCH < points.length) await sleep(12_000);
  }

  const result = { stepDeg, cells };
  await fs.mkdir(CACHE_DIR, { recursive: true });
