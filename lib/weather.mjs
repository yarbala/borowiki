// Погода из Open-Meteo (без ключа) и оценка готовности леса к грибам.
//
...
/** Плодовые тела появляются не сразу: пик примерно на девятый день, окно 3–16 дней. */
function timingFactor(d) {
  if (d == null || d < 0) return 0;
  if (d < 3) return 0.15 * (d / 3);
  if (d <= 9) return 0.15 + 0.85 * 0.5 * (1 - Math.cos((Math.PI * (d - 3)) / 6));
  if (d <= 16) return 0.5 * (1 + Math.cos((Math.PI * (d - 9)) / 7));
  return 0;
}

/** Насыщение около 30 мм: сверх этого лишняя вода уходит стоком. */
const rainFactor = (mm) => 1 - Math.exp(-Math.max(0, mm || 0) / 10);

/** Один ливень после засухи почти ничего не даёт: считаем баланс осадков и испарения. */
function moistureFactor(recentRainMm, et0Mm) {
  return 0.15 + 0.85 * clamp01(((recentRainMm || 0) - (et0Mm || 0) + 15) / 30);
}

function tempFactor(t) {
  if (t == null) return 0;
  if (t <= 4 || t >= 24) return 0;
  if (t < 10) return (t - 4) / 6;
  if (t <= 19) return 1;
  return (24 - t) / 5;
}

export function scoreSpot(daysSinceStorm, stormMm, recentRainMm, et0Mm, meanTempC, hadFrost) {
  const T = timingFactor(daysSinceStorm);
  const R = Math.pow(rainFactor(stormMm), 0.8);
  const M = Math.pow(moistureFactor(recentRainMm, et0Mm), 0.6);
  const W = tempFactor(meanTempC);
  return Math.max(0, Math.min(100, Math.round(100 * T * R * M * W * (hadFrost ? 0.35 : 1))));
}

/**
 * Оценки на сегодня, +3 и +7 дней плюс данные для карточки.
...
export function analyse(daily, stormDate, today) {
  ...
  const meanTemp = (from, to) => {
    let v = 0, n = 0;
    for (let i = Math.max(0, from); i <= Math.min(days.length - 1, to); i++) {
      if (daily.temperature_2m_max[i] == null) continue;
      v += (daily.temperature_2m_max[i] + daily.temperature_2m_min[i]) / 2;
      n++;
    }
    return n ? v / n : null;
  };
  ...
  const scores = [0, 3, 7].map((h) => {
    const e = t0 + h;
    if (e >= days.length) return 0;
    const frost = daily.temperature_2m_min.slice(Math.max(0, e - 5), e + 1).some((v) => v != null && v <= 0);
    return scoreSpot(e - s, stormMm, sum(daily.precipitation_sum, s + 1, e), sum(daily.et0_fao_evapotranspiration, s + 1, e), meanTemp(s + 1, e), frost);
  });

  // День, когда оценка максимальна — «пик» в карточке.
  let bestDay = t0, best = -1;
  for (let e = t0; e < Math.min(days.length, s + 17); e++) {
    const frost = daily.temperature_2m_min.slice(Math.max(0, e - 5), e + 1).some((v) => v != null && v <= 0);
    const sc = scoreSpot(e - s, stormMm, sum(daily.precipitation_sum, s + 1, e), sum(daily.et0_fao_evapotranspiration, s + 1, e), meanTemp(s + 1, e), frost);
    if (sc > best) { best = sc; bestDay = e; }
  }
