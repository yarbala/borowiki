...
export function analyse(daily, stormDate, today) {
  ...
  // Три дня вперёд служба отдаёт в том же ответе — раньше мы их просто выбрасывали.
  // Это прогноз, и в интерфейсе он отделён от измеренного.
  const forecast = [];
  for (let i = t0 + 1; i <= Math.min(days.length - 1, t0 + 3); i++) {
    forecast.push({
      date: days[i],
...
/**
 * Где в ближайшие дни ожидается гроза — сетка над лесом.
 *
 * Спрашивается только по нажатию кнопки в приложении: пока человек не попросил,
 * ни одного запроса не уходит. Шаг крупнее, чем у дождя: прогноз грозы всё равно
 * не точнее «в этом районе в этот день», и дробить его мельче — рисовать
 * несуществующую точность.
 *
 * @returns {Promise<{stepDeg:number, days:number, madeAt:string,
 *   cells:Array<[number,number,number,number]>}>} [lat, lon, через сколько дней, мм]
 */
export async function fetchStormForecast(bbox, { stepDeg = 0.2, days = 3, keep = () => true } = {}) {
  const points = [];
  ...
  for (let i = 0; i < points.length; i += BATCH) {
    ...
    const url =
      `${API}?latitude=${chunk.map((p) => p.lat).join(',')}` +
      `&longitude=${chunk.map((p) => p.lon).join(',')}` +
      `&daily=weather_code,precipitation_sum&timezone=Europe%2FWarsaw&forecast_days=${days + 1}`;
    ...
    chunk.forEach((p, j) => {
      ...
      // День 0 — сегодняшний, он уже наполовину прошёл; смотрим следующие.
      for (let k = 1; k <= days && k < d.weather_code.length; k++) {
        if ((d.weather_code[k] ?? 0) >= 95) {
          cells.push([p.lat, p.lon, k, Math.round((d.precipitation_sum[k] ?? 0) * 10) / 10]);
          break; // важен ближайший грозовой день
        }
      }
    });

    if (i + BATCH < points.length) await sleep(paceMs(weightOf(chunk.length, days + 1, 2)));
  }

  return { stepDeg, days, madeAt: new Date().toISOString(), cells, points: points.length };
}
