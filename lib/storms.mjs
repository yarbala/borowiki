...
export function buildPasses(frames) {
  ...
  for (const f of sorted) {
    ...
    for (const [x, y, i] of f.lit) {
      ...
      if (w > 0) {
        if (p.strongStartMs === undefined) p.strongStartMs = ms;
        p.strongEndMs = ms;
      }
      p.endMs = ms;
    }
    ...
    while (stack.length) {
      ...
      for (let dy = -r; dy <= r; dy++) {
        for (let dx = -r; dx <= r; dx++) {
          if (!dx && !dy) continue;
          const nx = c.x + dx;
          // Без этой проверки x = −1 даёт тот же номер, что последняя ячейка строки
          // выше: гроза у западного края региона склеивалась с грозой у восточного.
          if (nx < 0 || nx >= GRID.width) continue;
          const key = (c.y + dy) * GRID.width + nx;
