...
export async function loadAround(points, log = () => {}) {
  const tiles = new Set();
  for (const p of points) {
    const t = lonLatToTile(ZOOM, p.lon, p.lat);
    // Соседние тайлы нужны для точек у самого края.
    for (const dx of [-1, 0, 1]) for (const dy of [-1, 0, 1]) tiles.add(`${t.x + dx}|${t.y + dy}`);
  }
